...
// ═════════════════════════════════════════════
// BUILD LOGS
// Dated entries for work done after the deck was first submitted.
// Newest work goes at the bottom, so this reads in the order it happened.
// ═════════════════════════════════════════════

// ─────────────────────────────────────────────
// SLIDE: Build Logs Divider
// ─────────────────────────────────────────────
{
  let s = pres.addSlide();
  s.background = { color: C.dark };
  ...
  s.addText("Build Logs:", { x: 0.5, y: 1.2, w: 9, h: 1.0, fontSize: 52, bold: true, color: C.accent, fontFace: "Trebuchet MS", align: "center", valign: "middle", margin: 0 });
  s.addText("What Changed While Building It", { x: 0.5, y: 2.25, w: 9, h: 0.5, fontSize: 18, color: "A8D5BA", fontFace: "Calibri", align: "center", valign: "middle", margin: 0 });
  s.addText("Dated entries recording faults found, decisions made and parts redesigned", { x: 0.5, y: 2.8, w: 9, h: 0.4, fontSize: 13, color: "7EC8A0", fontFace: "Calibri", align: "center", italic: true, margin: 0 });

  addFooter(s, "Systems Engineering 3 & 4 Portfolio  |  AI Smart Bin Project");
}

// ─────────────────────────────────────────────
// BUILD LOG: the head was too tall
// ─────────────────────────────────────────────
{
  let s = pres.addSlide();
  s.background = { color: C.lightgray };
  addHeader(s, "Build Log - The Head Was Too Tall", "11 August 2026");

  // SUPERSEDED (left)
  s.addShape(pres.shapes.RECTANGLE, { x: 0.3, y: 1.2, w: 4.5, h: 4.0, fill: { color: C.white }, line: { color: "D1E8D9" }, shadow: mk() });
  s.addShape(pres.shapes.RECTANGLE, { x: 0.3, y: 1.2, w: 4.5, h: 0.42, fill: { color: "C0392B" }, line: { color: "C0392B" } });
  s.addText("SUPERSEDED: Bought Pan-Tilt - 147mm", { x: 0.3, y: 1.2, w: 4.5, h: 0.42, fontSize: 10.5, bold: true, color: C.white, fontFace: "Trebuchet MS", align: "center", valign: "middle", margin: 0 });

  s.addImage({ path: "images/cad/bought_tracker_head.png", x: 1.2, y: 1.7, w: 2.7, h: 2.03 });

  s.addText(
    [
      { text: "Measured against the assembly, not estimated", options: { bold: true, breakLine: true } },
      { text: "- The tray sat 147mm above the bin rim, and 85mm of that was the tracker stacking two servos on top of each other", options: { breakLine: true } },
      { text: "- Its tilt servo stands on end, so the servo body finishes 37mm above the tilt axis", options: { breakLine: true } },
      { text: "- The tray must clear that servo before it can clear anything else, which is what sets the height", options: { breakLine: true } },
      { text: "- Only the STL was available when it was bought, so it had been treated as fixed", options: { breakLine: true } },
    ],
    { x: 0.45, y: 3.8, w: 4.2, h: 1.35, fontSize: 9, color: C.text, fontFace: "Calibri", valign: "top", paraSpaceAfter: 1, margin: 0 }
  );

  // REBUILT (right)
  s.addShape(pres.shapes.RECTANGLE, { x: 5.1, y: 1.2, w: 4.6, h: 4.0, fill: { color: C.white }, line: { color: "D1E8D9" }, shadow: mk() });
  s.addShape(pres.shapes.RECTANGLE, { x: 5.1, y: 1.2, w: 4.6, h: 0.42, fill: { color: C.accent }, line: { color: C.accent } });
  s.addText("REBUILT: Own Design - 82mm", { x: 5.1, y: 1.2, w: 4.6, h: 0.42, fontSize: 10.5, bold: true, color: C.dark, fontFace: "Trebuchet MS", align: "center", valign: "middle", margin: 0 });

  s.addImage({ path: "images/cad/rebuilt_head_iso.png", x: 6.05, y: 1.7, w: 2.7, h: 2.03 });

  s.addText(
    [
      { text: "Replaced with three printed parts once the STEP file was obtained", options: { bold: true, breakLine: true } },
      { text: "- The pan servo now drops through the hub plate and hangs underneath, deleting the tracker's 125mm stand outright", options: { breakLine: true } },
      { text: "- The tilt servo moved out beside the tray instead of under it. The tray is only 90mm wide and tilts about a lengthways axis, so it can never sweep into it", options: { breakLine: true } },
      { text: "- Tray drops to 82mm above the rim, and the head uses 100.8cm3 of plastic against the 138.4cm3 it replaced", options: { breakLine: true } },
    ],
    { x: 5.25, y: 3.8, w: 4.3, h: 1.35, fontSize: 9, color: C.text, fontFace: "Calibri", valign: "top", paraSpaceAfter: 1, margin: 0 }
  );

  addFooter(s, "Build Log - Head Height");
}

// ─────────────────────────────────────────────
// BUILD LOG: checking it before printing
// ─────────────────────────────────────────────
{
  let s = pres.addSlide();
  s.background = { color: C.lightgray };
  addHeader(s, "Build Log - Checking It Before Printing", "11 August 2026");

  s.addText(
    "None of the parts had been printed, so the redesign was checked against the model rather than in the hand. Four faults were found this way, each of which would have meant a reprint.",
    { x: 0.3, y: 1.1, w: 9.4, h: 0.4, fontSize: 10.5, color: C.text, fontFace: "Calibri", valign: "top", margin: 0 }
  );

  s.addImage({ path: "images/cad/rebuilt_assembly_front.png", x: 0.3, y: 1.6, w: 4.4, h: 3.3 });
  s.addText("The rebuilt head sitting down inside the bin opening", { x: 0.3, y: 4.92, w: 4.4, h: 0.2, fontSize: 8, color: C.gray, fontFace: "Calibri", italic: true, align: "center", margin: 0 });

  s.addText(
    [
      { text: "Faults found before printing", options: { bold: true, breakLine: true } },
      { text: "- The pan ring's screws broke into the leg bracket's screw holes 2mm away. Caught because only 58% of the expected material came out of the cut", options: { breakLine: true } },
      { text: "- The servo's top face is a stepped stack standing 2.3mm proud of its case, which both new parts were sitting on", options: { breakLine: true } },
      { text: "- A 1mm rib runs down the centre of the servo flange, so the flange would have rocked on it instead of clamping flat", options: { breakLine: true } },
      { text: "- The tilt servo's flange fouled the yoke, so the servo could not drop into its mount", options: { breakLine: true } },
      { text: "", options: { breakLine: true } },
      { text: "How it was verified", options: { bold: true, breakLine: true } },
      { text: "- Interference analysis across all 14 parts: nothing touches", options: { breakLine: true } },
      { text: "- Re-run at 13 tilt angles. Clear through the full -60 to +60 degrees, with first contact at -65", options: { breakLine: true } },
      { text: "- At 60 degrees the tray's lowest corner measures 13.51mm against 13.54mm calculated by hand, so the model and the maths agree", options: { breakLine: true } },
    ],
    { x: 5.0, y: 1.6, w: 4.7, h: 3.6, fontSize: 8.5, color: C.text, fontFace: "Calibri", valign: "top", paraSpaceAfter: 1, margin: 0 }
  );

  addFooter(s, "Build Log - Verification");
}

// ─────────────────────────────────────────────
// BUILD LOG: hub plate over time
// ─────────────────────────────────────────────
{
  let s = pres.addSlide();
  s.background = { color: C.lightgray };
  addHeader(s, "Build Log - The Hub Plate Over Time", "Design Evolution");

  s.addImage({ path: "images/cad/evolution/plate.png", x: 0.3, y: 1.15, w: 9.4, h: 2.376 });

  s.addText(
    [
      { text: "These are not redrawings. ", options: { bold: true } },
      { text: "Each superseded version is rendered from the actual geometry stored in version control at the commit where it changed, so every panel is the part exactly as it was on that day. The plate is the part that changed most: it began as one piece with the pipe sockets hanging underneath, was split so each piece could print in its natural orientation, was cut down to a spoked wheel to save filament, had its bore opened after the real pan-tilt was measured, and has now lost that mounting entirely because the pan servo drops straight through it.", options: {} },
    ],
    { x: 0.3, y: 3.75, w: 9.4, h: 1.0, fontSize: 10, color: C.text, fontFace: "Calibri", valign: "top", margin: 0 }
  );

  s.addText(
    "Generated by cad/renders/ from the project's own version history",
    { x: 0.3, y: 4.95, w: 9.4, h: 0.2, fontSize: 8, color: C.gray, fontFace: "Calibri", italic: true, margin: 0 }
  );

  addFooter(s, "Build Log - Hub Plate Evolution");
}
